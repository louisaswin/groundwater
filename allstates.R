...
state <- "NC"
start_date <- as.Date("1940-10-31")  # 20 years ago from 2023
...
# need sites with a minimum of 10 years with observations
year_obvs = nc_gw1 %>%
  group_by(site_no) %>%
  summarize(years_of_observations = length(unique(year)))
head(year_obvs)
summary(year_obvs)
nrow(year_obvs)
...
insuff_sites = year_obvs %>%
  filter(years_of_observations < 10)
nrow(insuff_sites)

#do not use sites that have less than 10 years with observations
...
summary(year_obvs2)
nrow(year_obvs2)
# tracking only 283 sites now 


# have an observation from last 3 years
recent_obvs = nc_gw2 %>%
  filter(between(date, as.Date("2020-10-31"), as.Date("2023-10-31")))
recent_sites = unique(recent_obvs$site_no)
# filter only including the sites with recent obvs
# getting rid of lev_age_cd and lev_status_cd columns because they should all be A and 1 respectively
nc_gw3 = nc_gw2 %>% 
  filter(site_no %in% recent_sites) %>%
  select(-lev_age_cd, -lev_status_cd)
unique(nc_gw3$site_no)
#63 sites remaining
...
# dont think I want to filter on having at least one observation from first year
...
predict_site2 <- function(site_data) {
  ...
  # Predict for every 4 months in the span of 5 years (15 points)
  future_days <- seq(today() + 122, by = 122, length.out = 15) 
  
  # Create a data frame with 'days_since_start' for prediction
  newdata <- data.frame(days_since_start = as.numeric(future_days))
  
  # Predict using the model and newdata
...
long_df <- data.frame(
  site_no = rep(names(predictions_list2), each=15), 
  date = seq(today() + 122, by = 122, length.out = 15),
...
ggplot(long_df, aes(x=date, y=water_level, color=as.factor(site_no))) +
  geom_line() + 
